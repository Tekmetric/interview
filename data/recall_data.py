...
raw_data_path = dirname + "/raw_data/"
filename = raw_data_path + "Recalls_Data.csv"
s3_bucket_dirname = dirname + "/s3_bucket/"

if not os.path.exists(filename):
    raise Exception("Raw data folder doesn't exist")

if not os.path.exists(s3_bucket_dirname):
...
for i, row in enumerate(df.itertuples(index=False)):
    ...
    if iter_year != current_year or i == len(df) - 1:
        ...
        temp_df.to_parquet(s3_bucket_dirname + f"{current_year}.parquet")
