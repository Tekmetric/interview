...
// Helper functions
const responsivePadding = (isMobile) => isMobile ? 'px-2' : 'px-5';
const responsiveContainerPadding = (isMobile) => isMobile ? 'px-2.5' : 'px-10';
const baseCell = (isMobile, additionalClasses = '') =>
...
export const classes = {
  ...
  container: (isMobile) => `w-full min-h-screen bg-[#FF6B6B] dark:bg-slate-900 ${isMobile ? 'p-5 px-2.5' : 'p-10 px-5'} transition-colors duration-200`,
  ...
  header: (isMobile) => `bg-gradient-to-br from-[#FF6B6B] to-[#C92A2A] dark:from-slate-700 dark:to-slate-800 text-white ${isMobile ? 'p-6 px-5 pb-5' : 'p-10 pb-7'} transition-colors duration-200`,
  title: (isMobile) => `m-0 mb-2 font-bold tracking-tight ${isMobile ? 'text-[32px]' : 'text-[42px]'}`,
  tagline: (isMobile) => `m-0 mb-6 italic opacity-90 ${isMobile ? 'text-sm' : 'text-base'}`,
  ...
  tableHeader: (isMobile) => `flex font-semibold text-[#666] dark:text-gray-400 uppercase tracking-wider border-b-2 border-[#e0e0e0] dark:border-slate-600 pb-4 pt-6 items-center ${isMobile ? 'text-[10px]' : 'text-xs'} transition-colors duration-200`,
  tableHeaderCell: baseCell,
  tableHeaderCellImage: (isMobile) => baseCell(isMobile, 'justify-center'),

  listContainer: (isMobile) => `${isMobile ? 'px-2.5 pb-5' : 'px-10 pb-10'} outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:ring-inset flex-1 overflow-hidden`,

  row: 'flex border-b border-[#e0e0e0] dark:border-slate-600 hover:bg-[#f5f5f5] dark:hover:bg-slate-700 transition-colors pb-1',
  cellId: (isMobile) => baseCell(isMobile, 'justify-center dark:text-gray-300'),
  cellImage: (isMobile) => `flex flex-col items-center justify-center gap-1 ${responsivePadding(isMobile)} box-border`,
  cellName: baseCell,
  cellHeight: baseCell,
  cellWeight: baseCell,
  ...
  footer: (isMobile) => `text-center py-4 text-sm text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-slate-600 ${responsiveContainerPadding(isMobile)} transition-colors duration-200`
};
