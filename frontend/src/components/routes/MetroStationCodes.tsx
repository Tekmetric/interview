...
import { Route } from '@routes/metro.$stationCodes';
import { useSuspenseQuery } from '@tanstack/react-query';

export function MetroStationCodes() {
  ...
  let upcomingTrains = metroArrivalTimesQuery.data.Trains
    .filter(train => train.Line && train.Line !== 'No' && train.Line !== '--' && train.Min && train.Min !== '---' && train.DestinationName !== 'No Passenger')
  ...
  return (
    (stationCodes && stationCodes !== 'All' && !currentStation) || metroArrivalTimesQuery.error
      ? 'Station not found :('
      : (
              ...
              <TableBody>
                { upcomingTrains.length > 0
                  ? upcomingTrains.map(train => (
                      <TableRow
                        key={ `${ train.Line }-${ train.LocationCode }-${ train.Destination }-${ train.Min }-${ train.Group }` }
                      >
                        <TableCell className="flex align-center">
                          <Train
                            size="24"
                            className={ `${ METRO_COLORS[train.Line as keyof typeof METRO_COLORS] } rounded mr-2` }
                          />
                          <div>
                            { METRO_LINE_LABELS[train.Line as keyof typeof METRO_LINE_LABELS] } Line
                            to { train.DestinationName ?? train.Destination }
                          </div>
                        </TableCell>
                        <TableCell>
                  ...
                  : (
                      <TableRow>
                        <TableCell className="text-center" colSpan={ 2 }>No trains currently
                          scheduled.
                        </TableCell>
                      </TableRow>
                    ) }
