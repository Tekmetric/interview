...
class BlogEntryControllerTest {
    ...
    void shouldAcceptValidPaginationCall() {
        when(blogEntryService.getBlogEntries(any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(), PageRequest.of(0, 1), 1));

        Page<BlogEntryResponse> response = blogEntryController.getBlogEntries(Pageable.unpaged());
    ...
    void shouldFailPaginationCallWithInvalidSortField() {
        ResponseStatusException ex = assertThrows(ResponseStatusException.class, () -> {
            blogEntryController.getBlogEntries(Pageable.unpaged(Sort.by("not a value")));
        });
