...
class WorkOrderRestControllerIT {
    ...
    private static final String WORK_ORDER_ID = "00000000-0000-0000-0000-000000000021";
    private static final String PART_LINE_ITEM_ID = "00000000-0000-0000-0000-000000000031";
    private static final String LABOR_LINE_ITEM_ID = "00000000-0000-0000-0000-000000000041";
    ...
    void getAllWorkOrdersReturnsOk() throws Exception {
        ...
        mockMvc.perform(get("/work-orders"))
                ...
                .andExpect(content().json("""
                        ...
                        .formatted(WORK_ORDER_ID, CUSTOMER_ID, VEHICLE_ID)));
    ...
    void getAllWorkOrdersByCustomerIdReturnsOk() throws Exception {
        prepareForQuery();
        mockMvc.perform(get("/work-orders").param("customerId", CUSTOMER_ID))
                .andExpect(status().isOk())
                .andExpect(content().json("""
                        {"content":[{"id":"%s","customerId":"%s","vehicleId":"%s"}],"page":{"totalElements":1}}"""
                        .formatted(WORK_ORDER_ID, CUSTOMER_ID, VEHICLE_ID)));
    ...
    void getAllWorkOrdersByVehicleIdReturnsOk() throws Exception {
        prepareForQuery();
        mockMvc.perform(get("/work-orders").param("vehicleId", VEHICLE_ID))
                .andExpect(status().isOk())
                .andExpect(content().json("""
                        {"content":[{"id":"%s","customerId":"%s","vehicleId":"%s"}],"page":{"totalElements":1}}"""
                        .formatted(WORK_ORDER_ID, CUSTOMER_ID, VEHICLE_ID)));
    ...
    void getAllWorkOrdersByCustomerIdAndVehicleIdReturnsOk() throws Exception {
        prepareForQuery();
        mockMvc.perform(get("/work-orders").param("customerId", CUSTOMER_ID).param("vehicleId", VEHICLE_ID))
                .andExpect(status().isOk())
                .andExpect(content().json("""
                        {"content":[{"id":"%s","customerId":"%s","vehicleId":"%s"}],"page":{"totalElements":1}}"""
                        .formatted(WORK_ORDER_ID, CUSTOMER_ID, VEHICLE_ID)));
    ...
    void getWorkOrderByIdReturnsOk() throws Exception {
        prepareForQuery();
        mockMvc.perform(get("/work-orders/{id}", WORK_ORDER_ID))
                .andExpect(status().isOk())
                .andExpect(content().json("""
                        {"id":"%s","customerId":"%s","vehicleId":"%s",\
                        "partLineItems":[{"id":"%s","name":"Oil Filter","quantity":1,"partNumber":"00000000-0000-0000-0000-000000000101"}],\
                        "laborLineItems":[{"id":"%s","name":"Oil Change","quantity":1,"serviceCode":"00000000-0000-0000-0000-000000000201"}]}"""
                        .formatted(WORK_ORDER_ID, CUSTOMER_ID, VEHICLE_ID, PART_LINE_ITEM_ID, LABOR_LINE_ITEM_ID)));
    ...
    void createWorkOrderReturnsCreated() throws Exception {
        ...
        assertThat(persisted.getVehicle().getId()).isEqualTo(UUID.fromString(VEHICLE_ID));
    }

    @Test
    void updateWorkOrderReturnsOk() throws Exception {
        prepareForQuery();
        mockMvc.perform(put("/work-orders/{id}", WORK_ORDER_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        final WorkOrderEntity updated = workOrderRepository.findById(UUID.fromString(WORK_ORDER_ID)).orElseThrow();
    ...
    void deleteWorkOrderReturnsNoContent() throws Exception {
        prepareForQuery();
        mockMvc.perform(delete("/work-orders/{id}", WORK_ORDER_ID))
                .andExpect(status().isNoContent());

        entityManager.flush();
        entityManager.clear();
        assertThatQuery(statistics).hasDeleteCount(3).hasCollectionFetchCount(2).hasNoOtherOperations();

        assertThat(workOrderRepository.findById(UUID.fromString(WORK_ORDER_ID))).isEmpty();
    }

    @Test
    void createPartLineItemReturnsCreated() throws Exception {
        prepareForQuery();
        final String body = mockMvc.perform(post("/work-orders/{workOrderId}/part-line-items", WORK_ORDER_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        final WorkOrderEntity workOrder = workOrderRepository.findByIdWithPartLineItems(UUID.fromString(WORK_ORDER_ID)).orElseThrow();
    ...
    void deletePartLineItemReturnsNoContent() throws Exception {
        prepareForQuery();
        mockMvc.perform(delete("/work-orders/{workOrderId}/part-line-items/{lineItemId}", WORK_ORDER_ID, PART_LINE_ITEM_ID))
                .andExpect(status().isNoContent());
    ...
    void createLaborLineItemReturnsCreated() throws Exception {
        prepareForQuery();
        final String body = mockMvc.perform(post("/work-orders/{workOrderId}/labor-line-items", WORK_ORDER_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        final WorkOrderEntity workOrder = workOrderRepository.findByIdWithLaborLineItems(UUID.fromString(WORK_ORDER_ID)).orElseThrow();
    ...
    void deleteLaborLineItemReturnsNoContent() throws Exception {
        prepareForQuery();
        mockMvc.perform(delete("/work-orders/{workOrderId}/labor-line-items/{lineItemId}", WORK_ORDER_ID, LABOR_LINE_ITEM_ID))
                .andExpect(status().isNoContent());
    ...
    void deletePartLineItemReturnsNotFoundForUnknownWorkOrder() throws Exception {
        prepareForQuery();
        mockMvc.perform(delete("/work-orders/{workOrderId}/part-line-items/{lineItemId}", UUID.randomUUID(), PART_LINE_ITEM_ID))
                .andExpect(status().isNotFound());
        assertThatQuery(statistics).hasQueryCount(1).hasNoOtherOperations();
    }
    ...
    void deleteLaborLineItemReturnsNotFoundForUnknownWorkOrder() throws Exception {
        prepareForQuery();
        mockMvc.perform(delete("/work-orders/{workOrderId}/labor-line-items/{lineItemId}", UUID.randomUUID(), LABOR_LINE_ITEM_ID))
                .andExpect(status().isNotFound());
        assertThatQuery(statistics).hasQueryCount(1).hasNoOtherOperations();
    }
}
