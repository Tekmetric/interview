...
describe('getProducts', () => {
  ...
  it('throws HttpError on non-2xx responses', async () => {
    ...
    await expect(getProducts()).rejects.toThrow(HttpError);
  ...
  it('throws NetworkError when fetch rejects', async () => {
    ...
    await expect(getProducts()).rejects.toThrow(NetworkError);
  });
  ...
  it('throws InvalidResponseError when response is not valid JSON', async () => {
    ...
    await expect(getProducts()).rejects.toThrow(InvalidResponseError);
    await expect(getProducts()).rejects.toThrow('Response is not valid JSON');
  });
